...
def login(username, password):
    with open('db/user_credentials_db.txt') as file:
        ...
        for line in lines:
            usern, pswrd = line[:-1].split(", ")
            if usern == username and pswrd == password:
...
def render_login(errors=None):
    ...
    Label(text="Enter your password").grid(row=3, column=0)
    password = Entry(tk, show="*")
    password.grid(row=4, column=0)
    Button(tk, text='Enter', command=lambda: login(username.get(), password.get())).grid(row=5, column=0)
    if errors:
...
def render_register():
    ...
    Button(tk, text='Enter', command=lambda: register(
